...
```mermaid
flowchart TD
  A["fa:fa-robot Agent loop runtime"] --> B["fa:fa-folder-open Scan backlog/"]
  B --> C["fa:fa-list-check Pick next task"]
  C --> D["fa:fa-arrow-right Move task to wip/"]
  D --> E["fa:fa-book Read repository policies"]
  E --> F["fa:fa-code Implement changes"]
  F --> G["fa:fa-vial Run validations"]
  G --> H{"fa:fa-circle-question Validation passed?"}
  H -- "No" --> I["fa:fa-triangle-exclamation Capture failure context"]
  I --> J["fa:fa-ban Move to blocked/ or retry"]
  H -- "Yes" --> K["fa:fa-check Commit changes"]
  K --> L["fa:fa-box-archive Move task to done/"]
  L --> M["fa:fa-chart-line Generate metrics/report"]
  M --> B
...
```mermaid
flowchart LR
  A["fa:fa-user-gear Architects"] --> B["fa:fa-scale-balanced Policies"]
  B --> C["fa:fa-robot Agent runtime"]
  C --> D["fa:fa-vial Validation layer"]
  D --> E["fa:fa-code-pull-request Pull requests"]
  E --> F["fa:fa-user-check Human review"]
  F --> G["fa:fa-rocket Merge / deploy"]
...
```mermaid
flowchart LR
  A["fa:fa-code Agent change"] --> B["fa:fa-wand-magic-sparkles Lint"]
  B --> C["fa:fa-code-branch Type check"]
  C --> D["fa:fa-flask Unit tests"]
  D --> E["fa:fa-desktop E2E tests"]
  E --> F["fa:fa-shield-halved Security scan"]
  F --> G["fa:fa-circle-check Merge eligible"]
...
flowchart TD
  ...
  B1 --> C["fa:fa-vial Validation layer"]
  B2 --> C
  B3 --> C
  B4 --> C
  B5 --> C
  C --> D["fa:fa-user-check Human review"]
