...
這是一個摘記問題，因為答案不只是「抽出 common code」。有趣的部分是邊界：哪些決定應該成為 shared primitive，哪些決定應該留在產品應用裡，以及哪些 feedback signal 能證明 package 真的有幫助，而不是增加儀式。
...
這個摘記從重複工作開始，而不是從架構開始。好的 candidate 很容易辨識：在不同 app 間複製的 authentication wrapper、dashboard 裡重複的 chart normalization、每個 deployment target 都重寫的 environment helper，或因為上一版太靠近某個產品而被重新做的 test utility。
