...
3. **Maintenance:** an owner-maintained repository or a release during the observation window.
4. **Inspectability:** instructions and any referenced scripts can be read before use.
5. **Portability:** the format or package is documented for more than one client or surface. This is package-format portability, not a claim that one install location works everywhere.

Ties are resolved in this order: active maintenance, inspectability, then cross-client portability. These signals make a candidate representative of a visible workflow pattern; they do not prove popularity or effectiveness.
...
| --- | --- | --- | --- |
| Framing / specification | The [Superpowers workflow](https://github.com/obra/superpowers) places `brainstorming` before writing code and describes design refinement and approval; the selected file makes that gate explicit. | Requirements discovery, alternatives, design review, and a written spec become an explicit pre-implementation workflow. | This describes one maintained collection, not a universal team practice. |
...
| Verification / shipping | The selected [verification-before-completion skill](https://github.com/obra/superpowers/blob/main/skills/verification-before-completion/SKILL.md) makes fresh command output a precondition for completion claims; the [Superpowers release notes](https://github.com/obra/superpowers/blob/main/RELEASE-NOTES.md) record workflow and cross-platform worktree changes during the window. | Completion is reframed as a claim that needs newly collected evidence. | A passing command proves only the behavior that command covers. |
...
Five candidates were selected to cover the requested roles. Each meets the explicit-trigger and inspectability tests. The first four are in the same maintained public collection, which makes their related workflow assumptions readable together; the fifth is owner-published and intentionally domain-specific.

| Candidate and role | Trigger and source evidence | Maintenance / portability signal | Selection decision |
| --- | --- | --- | --- |
| [`brainstorming`](https://github.com/obra/superpowers/blob/main/skills/brainstorming/SKILL.md) — framing | Says to use before creative work that changes behavior. | [Superpowers](https://github.com/obra/superpowers) publishes multi-host installation guidance and a workflow catalog. | Selected for an explicit pre-code design gate. |
| [`test-driven-development`](https://github.com/obra/superpowers/blob/main/skills/test-driven-development/SKILL.md) — implementation discipline | Says to use before implementation code for features or bug fixes. | Same maintained, multi-host collection. | Selected for a concrete RED–GREEN–REFACTOR constraint. |
| [`systematic-debugging`](https://github.com/obra/superpowers/blob/main/skills/systematic-debugging/SKILL.md) — debugging / recovery | Says to use on bugs, test failures, and unexpected behavior before proposing fixes. | Same maintained, multi-host collection. | Selected for its staged root-cause workflow. |
| [`verification-before-completion`](https://github.com/obra/superpowers/blob/main/skills/verification-before-completion/SKILL.md) — review / verification | Says to use before claiming work complete, fixed, or passing. | Same collection; its [release notes](https://github.com/obra/superpowers/blob/main/RELEASE-NOTES.md) show active workflow maintenance in the window. | Selected as the final evidence gate, rather than treating a code-review prompt as equivalent to verification. |
| [`openai-docs`](https://github.com/openai/skills/blob/main/skills/.curated/openai-docs/SKILL.md) — domain knowledge | Its description targets OpenAI/Codex questions and requires official-source routing. | Owner-published in [OpenAI's skills catalog](https://github.com/openai/skills). | Selected as a narrow, inspectable vendor-domain skill; lower portability is expected and is part of the contrast. |
...
6. [Superpowers release notes](https://github.com/obra/superpowers/blob/main/RELEASE-NOTES.md) — maintenance evidence during the observation window.
